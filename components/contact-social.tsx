...
const ContactSocial = () => {
 ...
 const formRef = useRef<HTMLFormElement | null>(null);
  ...
  const socialHandles = [
    ...
    {
      label: "x.com/zitranet",
      icon:  <FaXTwitter color="white" size={16}/>,
      href: "https://x.com/@zitranet.com"
    },
    {
  ...
  return (
    <div
    className='w-full flex flex-col items-center justify-center px-2 mt-2 md:mt-8 gap-2'
   >
      <div className="w-full md:w-[90%] lg:w-[80%] flex flex-col md:flex-row items-start md:items-center md:justify-center gap-2">
          <div className="text-[15px] font-semibold text-neutral-800 text-center">
               Follow us on social to stay up-to-date on the latestt news, updates and promotions from zitranet
          </div>
      </div>
      <div className="w-full md:w-[90%] lg:w-[80%] flex flex-col md:flex-row items-start md:items-center md:justify-center gap-3">
      {
            socialHandles.map(con=>(
              ...
              className="flex items-center gap-2" >
                ...
                <span className="text-[15px] font-medium text-neutral-800">{con.label}</span>
              </a>
            ))
          }
      </div>
                ...
                <div className=" w-full flex items-center justify-center">
                    <form onSubmit={handleSubmit} ref={formRef}
                     className='flex ring-1 ring-pink-300 rounded-sm overflow-hidden w-full'>
                        <input type="email" placeholder='Your email address' name='email' required className='
                        ...
                        <div className="w-[50%]">
                           <Button
                            ...
                            className='w-full bg-slate-200 rounded-none text-pink-800 hover:bg-slate-300'>
                              {
                                loading? <div className="w-full gap-2 flex items-center justify-center">
                                 <Loader2 className=' animate-spin text-zinc-950'/>
                                  <span className='text-sm font-medium text-zinc-800 hidden md:flex'>sending...</span>
                                </div> :"Send"
                              }
                            </Button>
                        </div>
